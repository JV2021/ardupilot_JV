...
public:
    ...
    // run pcs bypass and send outputs to the motors. Added this JV
    void pcs_manual_bypass(float lateral_temp, float forward_temp, float yaw_rate_temp);

    // run pcs auto yaw controller. Added this JV
    ...
    void pcs_rf_controller(bool enabled_rfc, float plt_latitude, float plt_longitude, Vector3f dist_vec_tar_ned);     // Rotating force controller JV
    ...
    // static const struct AP_Param::GroupInfo var_infopcs[];      // Param JV
