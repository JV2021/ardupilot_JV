...
// Converts Thrust (N) to a cmd from -1 to +1. For BR2212 motor with 1045 prop. Added this JV
...
// Converts Thrust (N) to a cmd from -1 to +1. For BA2310-1220Kv motor with APC 7x5E prop. Added this JV
...
// Sets motor commands based on velocity target (latitude/longitude) from pilot. Added this JV
void AC_AttitudeControl_Multi::pcs_rf_controller(bool enabled_rfc, float plt_latitude, float plt_longitude, Vector3f dist_vec_tar_ned)           // Idle JV
{   // TODO JV: 1) Delete the saturations since the conversion model thrust to (-1 to +1) already does it.
    //       2) Clean the code by initializing the variables to 0 or a null vector.
    //       3) Consider the coupling in-between the inclination of the PCS (resultant thrust has a vertical component) and the required side thrust to remain at the centre.     
    ...
    if (enabled_rfc) {
        ...
        cmd_body.x = cmd_vel_latitude + cmd_pos.x + cmd_rf.x;                          // (North)
        cmd_body.y = cmd_vel_longitude + cmd_pos.y + cmd_rf.y;                         // (East)
    ...
    _motors.set_idle(enabled_rfc, _idle_on, cmd_idle);          // Set idle command
