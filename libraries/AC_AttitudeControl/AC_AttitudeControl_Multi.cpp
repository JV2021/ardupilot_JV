...
void AC_AttitudeControl_Multi::pcs_rf_controller(bool enabled_rfc, float plt_latitude, float plt_longitude, bool rc_idle)           // Idle JV
{   // TODO: Delete the saturations since the conversion model thrust to (-1 to +1) already does it. JV
    ...
    if (enabled_rfc) {
        ...
        if (_idle_on && rc_idle) {                 // Idle thrust setting (N). Idle JV
